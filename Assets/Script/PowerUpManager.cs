...
public class PowerUpManager : MonoBehaviour
{
    ...
    public int deleteSpawnInterval;
    private float timer;
    private float deleteSpawnTimer;
    ...
    private void Update()
    {
        ...
        {
            deleteSpawnTimer += Time.deltaTime;
        }
        if (deleteSpawnTimer > deleteSpawnInterval)
        {
            if (powerUpList.Count > 0)
            {
                RemovePowerUp(powerUpList[0]);
            }
            deleteSpawnTimer -= deleteSpawnInterval;
        }
    }
